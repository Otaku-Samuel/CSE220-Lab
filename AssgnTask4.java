...
class AssgnTask4{
    ...
    public static void rotateSecret( Character[][] board ){
        ...
        int x=board.length / 2;
            ...
            int rotation=1;
            while(j>=0){
                int count=board.length-1-j;
                for(int l=1; l<=rotation; l++){
                    Character temp = board[j][j];
                    for(int up=j; up<count; up++){
                        board[up][j]=board[up+1][j];
                    }
                    for(int left=j; left<count; left++){
                        board[count][left]=board[count][left+1];
                    }
                    for(int down=count; down>j; down--){
                        board[down][count]=board[down-1][count];
                    }
                    for(int right=count; right>j; right--){
                        board[j][right]=board[j][right-1];
                    }
                    board[j][j+1]=temp;
                }
                rotation++;
